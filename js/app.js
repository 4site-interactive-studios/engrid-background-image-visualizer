...
function recomputeCropFromFocal() {
  if (!state.image) return;
  state.crop = computeCropFromFocalPoint(state.image, state.focal, state.outputW, state.outputH);
...
async function applyImage(image) {
  ...
  if (saved) {
    ...
    state.quality = saved.quality ?? 80;
...
function wireCompression() {
  els.quality.addEventListener("input", () => {
    state.quality = clampInt(els.quality.value, 1, 100, 80);
    els.qualityOut.value = state.quality;
    scheduleEstimate();
    persistImageState();
  });
  ...
  const startCompare = (e) => {
    ...
    els.compareBtn.textContent = "Showing original";
    rerender();
  };
  const endCompare = () => {
  ...
  els.compareBtn.addEventListener("pointercancel", endCompare);
...
async function runEstimate() {
  if (!state.image || !state.crop || estimateInFlight) return;
